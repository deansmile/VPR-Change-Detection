...
sys.path.append(".")
sys.path.append("baselines/TransCD")
...
def yield_CD_evaluation(model, dataset, device="cuda", crop_shape=(504, 504)):
    ...
    for input_1, input_2, target in dataset:
        ...
        with torch.no_grad():
            predict = model(input_1, input_2)  # (batch, 1, m, n)
            predict = predict.squeeze(1)  # (batch, m, n)

        target = target.detach().cpu().numpy()  # (batch, m, n)
        predict = predict.detach().cpu().numpy()  # (batch, m, n)

        # similiar to TransCD/utils/evaluate.py -> confuse_matrix
        ...
        for pre, tar in zip(predict, target):
            ...
            # crop the results after inference
...
def wrap_eval_dataset(figsize=None):
    ...
    def wrapper(dataset, **kwargs):
        ...
        transform, target_transform = transform_loader(
            dataset,
            # copied from TransCD/utils/make_dataset.py -> OSCD_TRAIN
            mean=(0.5, 0.5, 0.5),
            std=(0.5, 0.5, 0.5),
            figsize=figsize,
        )
...
def main():
    ...
    pth_name = path.split("/")[-1].replace(".pth", "")
    config = networks.configs.CONFIGS[pth_name]

    model = networks.net.TransCDNet(
        config=config,
        img_size=512,
        vis=False,
    )
