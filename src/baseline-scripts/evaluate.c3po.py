...
sys.path.append(".")
sys.path.append("baselines/C-3PO/src")
...
def yield_CD_evaluation(model, dataset, device="cuda", crop_shape=(504, 504)):
    ...
    for input_1, input_2, target in dataset:
        ...
        with torch.no_grad():
            predict = model(x)["out"]  # (batch, 2, m, n)
...
def wrap_eval_dataset(figsize=None):
    ...
    def wrapper(dataset, **kwargs):
        ...
        transform, target_transform = transform_loader(
            dataset,
            mean=(0.485, 0.456, 0.406),  # copied from C3PO
            std=(0.229, 0.224, 0.225),  # copied from C3PO
            figsize=figsize,
        )
...
def main():
    ...
    model = C3PO.model_dict["resnet18_mtf_msf_deeplabv3"](args)

    model.load_state_dict(checkpoint["model"])
    model = model.to("cuda")
    model = model.eval()

    name = os.path.basename(best_pth).replace(".pth", "")
    eval_path = os.path.join(os.path.dirname(best_pth), f"eval.{name}.json")
